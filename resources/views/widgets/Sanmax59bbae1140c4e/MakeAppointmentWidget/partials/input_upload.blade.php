...
<div class="frm-ctrl-wrap">
    ...
    <button id="btn-{{$idModal}}" type="button" class="btn btn-success btn-md " style="display: block;">
        <i class="fa fa-upload"></i> Choose file...
    </button>
    <div id="btn-upload-{{$idModal}}-preview" class="img-res"
        ...
        <div class="mask">
            <a href="#" class="del-img"
               onclick="deleteImage('{{$idModal}}');return false;">
                <i class="fa fa-times" aria-hidden="true"></i>
            </a>
        </div>
    </div>
    <input type="hidden" id="{{$idModal}}" class="form-control"
           name="{{$input}}" value="{{ empty($model) ? old($input) : $model }}"
           placeholder="Choose file...">
</div>
<div class="modal fade" id="modal_{{$idModal}}" tabindex="-1" role="dialog" aria-labelledby="{{ $idModal }}Label">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <ul class="nav nav-tabs" role="tablist">
                    <li id="tab-{{$idModal}}-home" role="presentation" class="active">
                        <a href="#{{$idModal}}-home" aria-controls="{{$idModal}}-home" role="tab" data-toggle="tab">Upload
                            Image</a>
                    </li>
                    <li id="tab-{{$idModal}}-image" role="presentation">
                        <a href="#{{$idModal}}-image" aria-controls="{{$idModal}}-image" role="tab"
                           data-toggle="tab">Images</a>
                    </li>
                </ul>
                <div id="media-content" class="tab-content">
                    <div role="tabpanel" class="tab-pane" id="{{$idModal}}-home">
                        <div id="file-upload-{{$idModal}}" class="mydropzone dropzone">
                    ...
                    <div role="tabpanel" class="tab-pane" id="{{$idModal}}-image">
                        @include("widgets.{$theme}.{$widget}.partials.selectimage",['model'=>Admin::media(), 'selectImage' => $idModal])
                    </div>
                </div>
            </div>
...
<script>
    ...
    var myDropzone = new Dropzone("div#file-upload-{{$idModal}}", {
    ...
    myDropzone.on("queuecomplete", function (file, xhr, formData) {
        getPosts('{{ Admin::route("contentManager.media.images") }}', "{{$idModal}}");
    });

    $(document).ready(function () {
        $(document).on('click', '.pagination a', function (e) {
            getPosts($(this).attr('href'), "{{$idModal}}");
            e.preventDefault();
        });
        $("#btn-{{$idModal}}").on('click', function () {
            defaultActive("{{$idModal}}");
